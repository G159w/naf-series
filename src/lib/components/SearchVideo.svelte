<script lang="ts">
	...
	const getUrlSearchType = () => {
		...
		if (!type || (type !== 'title' && type !== 'stars' && type !== 'creators')) {
			return 'title';
		}
		return type;
	};
...
<div class="card w-48 shadow-xl rounded-md py-2 z-[999]" data-popup="comboVideoType">
	<ListBox rounded="rounded-none">
		<ListBoxItem bind:group={videoType} name={'Nothing'} value={undefined}>Tout</ListBoxItem>
...
>
	<ListBox rounded="rounded-none">
		<ListBoxItem bind:group={genre} name={'Nothing'} value={undefined}>Tout</ListBoxItem>
...
<form class="shadow-lg">
	<div class="flex flex-row input-group input-group-divider w-fit h-12 rounded-md">
		<button type="button" class=" ghost-filled w-24" use:popup={searchTypeCombobox}>
			{searchTypeComboValues[searchType]}
		</button>
		<input bind:value={searchText} class="w-[300px]" type="search" placeholder="Recherche ..." />

...
<div on:focus on:mouseover on:click  on:focus on:mouseleave class="relative w-6 h-6">
  <div on:mouseover={() => onHover(0)} on:click={() => onClick(0)} on:focus class="absolute top-0 left-0 w-3 h-6 z-[1] " />
  <div on:mouseover={() => onHover(0.5)} on:click={() => onClick(0.5)} on:focus class="absolute top-0 left-3 w-3 h-6 z-[1]" />
  <svg
